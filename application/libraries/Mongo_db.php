...
Class Mongo_db
{
     ...
     */
    const READ_PREFERENCE_MODE = ReadPreference::RP_NEAREST;
     ...
     *  @return    $this
     */
    public function order_by($fields, $sort = 1): self
    {
     ...
     *  @return   integer
     */
    public function count(string $collection = '', array $options = []) {
        ...
        $resultCount = $this->_command($command, __METHOD__);
        // Query reset.
        if ($this->auto_reset_query === TRUE) {
            $this->reset_query();
        }
     ...
     *  @return  MongoDB\Driver\Manager
     */
    public function switch_db(string $database = ''): self
    {
        if ($database == '')
        {
    ...
    private function prepare(): self
    {
        ...
        // Database name.
        if (isset($connection['db_auth']) && is_string($connection['db_auth']) && $connection['db_auth'] != '')
        {
            $this->db_name = trim($connection['db_auth']);
        }
        if (isset($connection['db_name']) && is_string($connection['db_name']) && $connection['db_name'] != '')
        {
            $this->db_name = trim($connection['db_name']);
        }
        if ($this->auth === TRUE && !isset($connection['db_auth']) && !isset($connection['db_name']))
        {
    ...
    private function create_connection_string(): string
    {
        ...
        if ($this->auth === TRUE && $this->user_name != '')
        {
            ...
            {
                $result .= ":".urlencode($this->user_password);
            }
         ...
         */
        if($this->auth === TRUE) {
            if(!empty($this->db_auth)) {
                $result .= "/{$this->db_auth}";
            } elseif(!empty($this->db_auth))  {
                $result .= "/{$this->db_name}";
            }
        }
	...
	*/
	function listCollections() : array
	{
		...
		$res = $this->db->executeCommand($this->db_name, $listdatabases);
		$listCollections = [];
     ...
     * set to 1 (ASC).
     *
     * @usage : add_index($collection, array('first_name' => 'ASC', 'last_name' => -1), array('unique' => TRUE));
     */
    public function add_index($collection = "", $keys = array(), $options = array()) {
        ...
        foreach ($keys as $name => $value) {
            $command["indexes"][] = ["name" => $name . "_" . $value, "key" => ["$name" => $value], "ns" => "databaseName.collectionName"];
        }
        return $this->_command($command, __METHOD__);
    }
     ...
     *
     * @usage : remove_index($collection, array('first_name', 'last_name'));
     */
    public function remove_index($collection = "", $keys = array()) {
        //{ dropIndexes: "collection", index: [ "age_1", "age_1_status_1" ] } 
        $command = [
     ...
     *
     * @usage : $instance->batch_insert('foo', $data = array());
     */
    public function batch_insert($collection = "", $insert = array()) {
        return $this->insertAll($collection, $insert);
    }
     ...
     *
     * @usage: $instance->distinct('collection', 'field');
     */
    public function distinct($collection = "", $field = "") {
        ...
        $data = $this->_command($command, __METHOD__);
    ...
    /**
     * --------------------------------------------------------------------------------
     * //! Drop collection [NNBINH]
     * --------------------------------------------------------------------------------
     *
     * Drop a Mongo collection
     * @usage: $instance->drop_collection('bar');
     */
    ...
    /**
     * --------------------------------------------------------------------------------
     * //! Drop database [NNBINH]
     * --------------------------------------------------------------------------------
     *
     * Drop a Mongo database
     * @usage: $instance->drop_db("foobar");
     */
     ...
     *
     * 	@usage: $instance->update_all('foo', $data = array());
     */
    public function update_all($collection = "", $data = array(), $options = array()): bool {
        return $this->update($collection, $data, array_merge($options, ["multi" => TRUE, 'upsert' => FALSE]));
    }
     ...
     *
     * @usage : $instance->where_in_all('foo', array('bar', 'zoo', 'blah'))->get('foobar');
     */
    public function where_in_all($field = "", $in = array()): self {
        if (empty($field)) {
            $this->error("Mongo field is require to perform where all in query.", __METHOD__);
        }
        ...
        if (is_array($in) && count($in) > 0) {
            ...
            $this->error("in value should be an array.", __METHOD__);
        }
    }
     ...
     *
     * 	@usage : $instance->where_not_in('foo', array('bar', 'zoo', 'blah'))->get('foobar');
     */
    public function where_not_in($field = "", $in = array()): self {
        return $this->where_nin($field, $in);
    }
     ...
     *
     * 	@usage : $instance->where_near('foo', array('50','50'))->get('foobar');
     */
    function where_near($field = '', $co = array()) {
        ...
        if (is_array($co) && count($co) > 0) {
            ...
            $this->error("co value should be an array.", __METHOD__);
        }
    }
     ...
     * @return array
     */
    public function aggregate_pipline($collection = "", $pipeline = array(), $returns = array(), $options = array()): array {

        $result = $this->aggregate($collection, $pipeline, $options);

        $finalreturns = array_merge($returns, $result);
        return $finalreturns;
    }
