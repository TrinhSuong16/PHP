 ...
 *
 * @package   CI WFF_Smarty
 * @author    MitsuyoRai
 * @license   MIT
 * @version   3.0
 */

class WFF_Parser extends CI_Parser  {
    ...
    public function login($data = array(), $return = FALSE) {
        return $this->parse("templates/login", $data, $return);
    }

    public function layouts($data = array(), $return = FALSE) {
        
        $type = isset($data["themes"]) ? $data["themes"] : "horizontal";
        return $this->parse("templates/customer/{$type}/layouts", $data, $return);
    }

    public function error($data = array(), $code = 404) {
        $this->CI->output->set_status_header($code);
        return $this->parse("templates/errors/e".$code, $data, FALSE);
    }
    ...
    protected function _parse($template, $data, $return = FALSE)
    {
        ...
        foreach ($replace as $from => $to) {
            $template = str_ireplace($from, ( ! is_null($to) ? $to : ''), $template);
        }
		...
		if(isset($matches[0])){
			...
			foreach ($data as $pos => $row) {
				if(!is_object($row) && !is_array($row)){
					$replace[$this->l_delim.$variable.".".$pos.$this->r_delim] = $row;
				} else {
                    $pair = $this->_parse_pair($variable.".".$pos, $row, $string);
                    $replace = array_merge($replace, $pair);
                }
			}
		}
        return $replace;
